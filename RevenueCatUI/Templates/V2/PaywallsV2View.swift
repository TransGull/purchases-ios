...
//            }
    ...
    static func chooseLocalization(
        ...
        var chosenLocale = Self.preferredLocale(from: paywallLocales, preferredLocales: preferredLocales)
        ?? defaultLocale
        if let identifier = UserDefaults.standard.string(forKey: "AppLanguage") {
            chosenLocale = Locale(identifier: identifier)
        }
